...
12. **Prompt versioning (Phoenix)** — Arize Phoenix as a single-container prompt registry + observability stack (`infra/observability/`, image `arizephoenix/phoenix:20.3.0`, SQLite backend, on `llm-net`). Prompts are declared in `seed/prompts/manifest.toml` (logical key → name/label/file) with text in `seed/prompts/*.md`. `seed_prompts.py` idempotently registers each entry into Phoenix and tags it (`production`). `prompts.py` owns registry access: `load_manifest()` + `fetch_system_prompt(key)` (fetch the version at the entry's label, fall back to the seed file if Phoenix is unreachable). `QueryRewriter` fetches its prompt at init (cached), with a `system_prompt` injection seam so tests need no live Phoenix. Only `PHOENIX_ENDPOINT` is in `.env`; name/label live in the manifest. Source of truth is Phoenix after seeding; UI edits go live by re-pointing the label. See [observability.md](observability.md). ✅ Done (live fetch + offline tests verified)

13. **Company resolver (dummy) + pipeline composition** — `company_resolver.py` `CompanyResolver`: surface form → canonical id, deterministic and separate from the rewrite LLM. `resolve(list[str])` returns per-input `Resolution{input, outcome (resolved/not_found/ambiguous), canonical?, match_type?, candidates?}` matching the documented contract. Current implementation is a dummy: exact lookup against a hardcoded Apple registry (no alias/fuzzy/ambiguous yet) — placeholder so the call site exists. `pipeline.py` `QueryPipeline.run()` composes the online path so far: query rewrite → resolve `filters.company` → `ResolvedQuery{rewrite, companies}`; both stages injectable. Tests: `test_company_resolver.py` (exact/case-insensitive/not_found/order) and `test_pipeline.py` (stub rewriter, wiring + empty-filter guard), both hermetic. See [query-rewrite-strategy.md](query-rewrite-strategy.md). ✅ Done (in-container end-to-end run verified: Apple → AAPL)
...
- **Online path — query rewrite, prompt versioning (Phoenix), and a dummy company resolver + pipeline are done.** Next in the sequence: **ingestion metadata** — stamp `company`/`doc_type`/`period`/`version` onto each chunk at embed time, so the resolver and metadata-filtered retrieval have real fields to work with. Then **upgrade the company resolver** from the dummy dict to a real registry built from ingested EDGAR CIK/ticker/name, with alias/fuzzy matching and `ambiguous` outcomes. Then **retrieval** (metadata-filtered similarity search). See [query-rewrite-strategy.md](query-rewrite-strategy.md) for the resolver contract and open items.
