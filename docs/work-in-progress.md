...
- **Ingestion metadata + config/registry are done** (step 14), and the corpus was re-indexed (415 vectors, all filter fields verified). To rebuild from scratch again: `docker compose -f infra/ingestion/docker-compose.yml up -d --build` then `... exec app uv run dagster asset materialize --select '*' -m financial_doc_ai.definitions` (embed stage ~10 min on CPU). Next in the sequence: **upgrade the company resolver** from the dummy dict to use the real registry (`data/reference/company_tickers.json` now exists) with alias/fuzzy matching and `ambiguous` outcomes. Then **retrieval** (metadata-filtered similarity search over the now-stamped fields). See [query-rewrite-strategy.md](query-rewrite-strategy.md) for the resolver contract and open items.
